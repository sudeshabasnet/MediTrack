...
async def run_test():
    ...
    try:
        ...
        # -> Fill in all required fields with valid data for General User registration
        frame = context.pages[-1]
        # Fill Full Name with 'Test User'
        ...
        frame = context.pages[-1]
        # Fill Confirm Password with 'TestPassword123!'
        ...
        # -> Check for any email verification or other confirmation outside the form or try to navigate to login page to verify if user can login after registration
        frame = context.pages[-1]
        # Click on the Login link to check if the user can login after registration
        elem = frame.locator('xpath=html/body/div/div[2]/nav/div/div/div[2]/a[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Attempt to login with the registered user's credentials to verify successful registration
        frame = context.pages[-1]
        # Input registered user's email in login form
        elem = frame.locator('xpath=html/body/div/div[2]/div/div/form/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('testuser@example.com')
        

        frame = context.pages[-1]
        # Input registered user's password in login form
        elem = frame.locator('xpath=html/body/div/div[2]/div/div/form/div[2]/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('TestPassword123!')
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Registration Successful! Welcome, Test User').first).to_be_visible(timeout=30000)
