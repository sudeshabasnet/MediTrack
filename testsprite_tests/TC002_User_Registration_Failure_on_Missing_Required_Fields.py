...
async def run_test():
    ...
    try:
        ...
        # -> Attempt to submit registration form with all required fields empty for General User role.
        frame = context.pages[-1]
        # Click Register button to submit empty form for General User role
        elem = frame.locator('xpath=html/body/div/div[2]/div/div/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Complete the task by confirming validation errors prevent registration and then stop.
        frame = context.pages[-1]
        # Click Register button to attempt submission with Pharmacy role and empty license number to confirm validation prevents registration
        elem = frame.locator('xpath=html/body/div/div[2]/div/div/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        await expect(frame.locator('text=License Number *').first).to_be_visible(timeout=30000)
