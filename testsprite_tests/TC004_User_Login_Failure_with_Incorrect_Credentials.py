...
async def run_test():
    ...
    try:
        ...
        # -> Check browser localStorage and sessionStorage for JWT token presence after invalid login attempt.
        frame = context.pages[-1]
        # Click login button again to see if error message appears
        elem = frame.locator('xpath=html/body/div/div[2]/div/div/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        await expect(frame.locator('text=Email / Username').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=Password').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=Logging in...').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=Don\'t have an account? Register New Account').first).to_be_visible(timeout=30000)
