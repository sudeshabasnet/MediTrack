...
async def run_test():
    ...
    try:
        ...
        # Click on the Login link at the top to go to login page
        ...
        # -> Try login with User role credentials if available or request valid credentials for all roles.
        frame = context.pages[-1]
        # Input User email
        elem = frame.locator('xpath=html/body/div/div[2]/div/div/form/div/input').nth(0)
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Welcome to the Nonexistent Dashboard').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test plan execution failed: Users were not redirected to their specific dashboards based on their roles (Admin, User, Supplier, Pharmacy). This assertion fails immediately to indicate the failure.')
