...
async def run_test():
    ...
    try:
        ...
        # -> Input general user email and password and submit login form
        frame = context.pages[-1]
        ...
        await page.wait_for_timeout(3000); await elem.fill('generaluserpassword')
        ...
        # -> Retry login as general user with correct credentials or check for error messages
        frame = context.pages[-1]
        # Re-input general user email
        elem = frame.locator('xpath=html/body/div/div[2]/div/div/form/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('generaluser@example.com')
        

        frame = context.pages[-1]
        # Re-input general user password
        elem = frame.locator('xpath=html/body/div/div[2]/div/div/form/div[2]/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('generaluserpassword')
        

        frame = context.pages[-1]
        # Click login button again to submit login form
        elem = frame.locator('xpath=html/body/div/div[2]/div/div/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click the login button to submit the general user login form
        frame = context.pages[-1]
        # Click login button to submit general user login form
        elem = frame.locator('xpath=html/body/div/div[2]/div/div/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Clear the email and password fields if possible, then input valid admin credentials and submit login form
        frame = context.pages[-1]
        ...
        try:
            ...
            raise AssertionError('Test failed: General user was able to access admin or supplier dashboards without permission, violating access control rules.')
