...
async def run_test():
    ...
    try:
        ...
        # Click on Login link to start Admin login process
        ...
        # -> Retry login or check for errors, or try to navigate to medicine management page if possible.
        frame = context.pages[-1]
        ...
        # -> Try to clear the input fields and re-enter Admin credentials, then submit login again.
        frame = context.pages[-1]
        ...
        # -> Try to login as Supplier user to test medicine deletion or investigate alternative navigation to medicine management page.
        frame = context.pages[-1]
        # Click Register link to check if there is an option to login as Supplier or navigate to medicine management page alternatively
        elem = frame.locator('xpath=html/body/div/div[2]/nav/div/div/div[2]/a[3]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click Login link on registration page to navigate back to login form for Supplier login.
        frame = context.pages[-1]
        # Click Login link on registration page to go back to login form
        elem = frame.locator('xpath=html/body/div/div[2]/div/div/div[2]/p/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Input Supplier email and password, then submit login form.
        frame = context.pages[-1]
        ...
        try:
            ...
            raise AssertionError("Test case failed: The test plan to verify Admin and Supplier can delete medicines and that deleted medicines no longer appear in inventory has failed. The expected confirmation text 'Medicine Deletion Successful' was not found on the page, indicating the deletion did not occur or was not reflected in the inventory.")
