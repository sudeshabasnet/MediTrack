...
async def run_test():
    ...
    try:
        ...
        # -> Click on Login to start login as general user
        ...
        # -> Refill registration form with new general user details, ensure terms checkbox is checked, and submit again
        frame = context.pages[-1]
        # Input Full Name for new general user
        elem = frame.locator('xpath=html/body/div/div[2]/div/div/form/div/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('Test User')
        ...
        # -> Input email and password for existing general user and click login button
        frame = context.pages[-1]
        # Input existing general user email
        elem = frame.locator('xpath=html/body/div/div[2]/div/div/form/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('testuser@example.com')
        

        frame = context.pages[-1]
        # Input existing general user password
        elem = frame.locator('xpath=html/body/div/div[2]/div/div/form/div[2]/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('password123')
        ...
        # -> Click login button to submit login form and login as general user
        frame = context.pages[-1]
        # Click login button to submit login form
        elem = frame.locator('xpath=html/body/div/div[2]/div/div/form/div[2]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click login button (index 10) to submit login form and login as general user
        frame = context.pages[-1]
        # Click login button to submit login form
        elem = frame.locator('xpath=html/body/div/div[2]/div/div/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        try:
            ...
            raise AssertionError("Test case failed: The test plan execution failed to verify that a general user can add medicine items to their cart and the cart updates accordingly.")
