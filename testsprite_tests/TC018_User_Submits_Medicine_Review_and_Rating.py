...
async def run_test():
    ...
    try:
        ...
        # -> Click on the Login link to start login process as general user
        ...
        # -> Check if there is a Register or Forgot Password option to create or recover a valid user account
        frame = context.pages[-1]
        # Click on Register button to create a new user account
        elem = frame.locator('xpath=html/body/div/div[2]/nav/div/div/div[2]/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click on Login link to attempt login as general user
        frame = context.pages[-1]
        # Click on Login link to open login form
        elem = frame.locator('xpath=html/body/div/div[2]/nav/div/div/div[2]/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        try:
            ...
            raise AssertionError('Test case failed: The review submission and validation process did not complete successfully as per the test plan. Review is not visible on the medicine page.')
